...
B3D.GLTF2Loader = ( function () {
	...
	GLTF2Loader.prototype = {
		...
		load: function ( url, onLoad, onProgress, onError ) {
			...
			var path = this.path && ( typeof this.path === "string" ) ? this.path : B3D.Loader.prototype.extractUrlBase( url );
	...
	function GLTFLightsExtension( json ) {
		...
		for ( var lightId in lights ) {
			...
			var color = new B3D.Color().fromArray( light.color );

			switch ( light.type ) {

				case 'directional':
					lightNode = new B3D.DirectionalLight( color );
					lightNode.position.set( 0, 0, 1 );
					break;

				case 'point':
					lightNode = new B3D.PointLight( color );
					break;

				case 'spot':
					lightNode = new B3D.SpotLight( color );
					lightNode.position.set( 0, 0, 1 );
					break;

				case 'ambient':
					lightNode = new B3D.AmbientLight( color );
					break;
	...
	GLTFMaterialsCommonExtension.prototype.getMaterialType = function ( material ) {
		...
		switch ( khrMaterial.type ) {
			...
			case 'commonBlinn' :
			case 'commonPhong' :
				return B3D.MeshPhongMaterial;

			case 'commonLambert' :
				return B3D.MeshLambertMaterial;

			case 'commonConstant' :
			default :
				return B3D.MeshBasicMaterial;
	...
	GLTFMaterialsCommonExtension.prototype.extendParams = function ( materialParams, material, dependencies ) {
		...
		if ( materialValues.diffuseFactor !== undefined ) {
			...
			materialParams.color = new B3D.Color().fromArray( materialValues.diffuseFactor );
		...
		if ( materialValues.specularFactor !== undefined ) {
			...
			materialParams.specular = new B3D.Color().fromArray( materialValues.specularFactor );
	...
	GLTFTechniqueWebglExtension.prototype.extendParams = function ( materialParams, material, dependencies ) {
		...
		materialParams.vertexShader = replaceB3DShaderAttributes( vertexShader, technique );
		...
		if ( enableCullFace ) {
			...
			materialParams.side = functions.cullFace !== undefined ? WEBGL_SIDES[ functions.cullFace ] : B3D.FrontSide;

		} else {

			materialParams.side = B3D.DoubleSide;

		}

		materialParams.depthTest = enableDepthTest;
		materialParams.depthFunc = functions.depthFunc !== undefined ? WEBGL_DEPTH_FUNCS[ functions.depthFunc ] : B3D.LessDepth;
		materialParams.depthWrite = functions.depthMask !== undefined ? functions.depthMask[ 0 ] : true;

		materialParams.blending = enableBlend ? B3D.CustomBlending : B3D.NoBlending;
		...
		if ( blendEquationSeparate !== undefined ) {
			...
			materialParams.blendEquation = B3D.AddEquation;
			materialParams.blendEquationAlpha = B3D.AddEquation;
		...
		if ( blendFuncSeparate !== undefined ) {
			...
			materialParams.blendSrc = B3D.OneFactor;
			materialParams.blendDst = B3D.ZeroFactor;
			materialParams.blendSrcAlpha = B3D.OneFactor;
			materialParams.blendDstAlpha = B3D.ZeroFactor;
	...
	function GLTFMaterialsPbrSpecularGlossinessExtension() {
		...
		return {
			...
			extendParams: function ( params, material, dependencies ) {
				...
				var shader = B3D.ShaderLib[ 'standard' ];

				var uniforms = B3D.UniformsUtils.clone( shader.uniforms );
				...
				uniforms.specular = { value: new B3D.Color().setHex( 0x111111 ) };
				...
				params.color = new B3D.Color( 1.0, 1.0, 1.0 );
				params.opacity = 1.0;
				...
				params.emissive = new B3D.Color( 0.0, 0.0, 0.0 );
				params.glossiness = pbrSpecularGlossiness.glossinessFactor !== undefined ? pbrSpecularGlossiness.glossinessFactor : 1.0;
				params.specular = new B3D.Color( 1.0, 1.0, 1.0 );
			...
			createMaterial: function ( params ) {
				...
				material.normalScale = new B3D.Vector2( 1, 1 );
	...
	var WEBGL_FILTERS = {
		9728: B3D.NearestFilter,
		9729: B3D.LinearFilter,
		9984: B3D.NearestMipMapNearestFilter,
		9985: B3D.LinearMipMapNearestFilter,
		9986: B3D.NearestMipMapLinearFilter,
		9987: B3D.LinearMipMapLinearFilter
	};

	var WEBGL_WRAPPINGS = {
		33071: B3D.ClampToEdgeWrapping,
		33648: B3D.MirroredRepeatWrapping,
		10497: B3D.RepeatWrapping
	};

	var WEBGL_TEXTURE_FORMATS = {
		6406: B3D.AlphaFormat,
		6407: B3D.RGBFormat,
		6408: B3D.RGBAFormat,
		6409: B3D.LuminanceFormat,
		6410: B3D.LuminanceAlphaFormat
	};

	var WEBGL_TEXTURE_DATATYPES = {
		5121: B3D.UnsignedByteType,
		32819: B3D.UnsignedShort4444Type,
		32820: B3D.UnsignedShort5551Type,
		33635: B3D.UnsignedShort565Type
	};

	var WEBGL_SIDES = {
		1028: B3D.BackSide,  // Culling front
		1029: B3D.FrontSide  // Culling back
		//1032: B3D.NoSide   // Culling front and back, what to do?
	};
	...
	var WEBGL_DEPTH_FUNCS = {
		...
		513: B3D.LessDepth,
		514: B3D.EqualDepth,
		515: B3D.LessEqualDepth,
		516: B3D.GreaterEqualDepth,
		517: B3D.NotEqualDepth,
		518: B3D.GreaterEqualDepth,
		519: B3D.AlwaysDepth
	};

	var WEBGL_BLEND_EQUATIONS = {
		32774: B3D.AddEquation,
		32778: B3D.SubtractEquation,
		32779: B3D.ReverseSubtractEquation
	};

	var WEBGL_BLEND_FUNCS = {
		0: B3D.ZeroFactor,
		1: B3D.OneFactor,
		768: B3D.SrcColorFactor,
		769: B3D.OneMinusSrcColorFactor,
		770: B3D.SrcAlphaFactor,
		771: B3D.OneMinusSrcAlphaFactor,
		772: B3D.DstAlphaFactor,
		773: B3D.OneMinusDstAlphaFactor,
		774: B3D.DstColorFactor,
		775: B3D.OneMinusDstColorFactor,
		776: B3D.SrcAlphaSaturateFactor
	...
	var INTERPOLATION = {
		LINEAR: B3D.InterpolateLinear,
		STEP: B3D.InterpolateDiscrete
	};
	...
	function replaceB3DShaderAttributes( shaderText, technique ) {
	...
	function createDefaultMaterial() {
		...
		return new B3D.MeshPhongMaterial( {
			color: 0x00000,
			emissive: 0x888888,
			specular: 0x000000,
			shininess: 0,
	...
	DeferredShaderMaterial.prototype.create = function () {
		...
		var uniforms = B3D.UniformsUtils.clone( this.params.uniforms );

		for ( var uniformId in this.params.uniforms ) {

			var originalUniform = this.params.uniforms[ uniformId ];

			if ( originalUniform.value instanceof B3D.Texture ) {
		...
		return new B3D.RawShaderMaterial( this.params );
	...
	GLTFParser.prototype.loadBuffers = function () {
		...
		return _each( json.buffers, function ( buffer, name ) {
			...
			if ( buffer.type === 'arraybuffer' || buffer.type === undefined ) {
				...
				console.warn( 'B3D.GLTF2Loader: ' + buffer.type + ' buffer type is not supported' );
		...
		return this._withDependencies( [
			...
			return _each( json.accessors, function ( accessor ) {
				...
				if ( accessor.byteStride && accessor.byteStride !== itemBytes ) {
					...
					var ib = new B3D.InterleavedBuffer( array, accessor.byteStride / elementBytes );

					return new B3D.InterleavedBufferAttribute( ib, itemSize, accessor.byteOffset / elementBytes );

				} else {

					array = new TypedArray( arraybuffer, accessor.byteOffset, accessor.count * itemSize );

					return new B3D.BufferAttribute( array, itemSize );
			...
			return _each( json.textures, function ( texture ) {
				...
				if ( texture.source !== undefined ) {
					...
					return new Promise( function ( resolve ) {
						...
						var textureLoader = B3D.Loader.Handlers.get( sourceUri );

						if ( textureLoader === null ) {

							textureLoader = new B3D.TextureLoader();
						...
						textureLoader.load( resolveURL( sourceUri, options.path ), function ( _texture ) {
							...
							_texture.format = texture.format !== undefined ? WEBGL_TEXTURE_FORMATS[ texture.format ] : B3D.RGBAFormat;

							if ( texture.internalFormat !== undefined && _texture.format !== WEBGL_TEXTURE_FORMATS[ texture.internalFormat ] ) {

								console.warn( 'B3D.GLTF2Loader: Three.js doesn\'t support texture internalFormat which is different from texture format. ' +
															'internalFormat will be forced to be the same value as format.' );

							}

							_texture.type = texture.type !== undefined ? WEBGL_TEXTURE_DATATYPES[ texture.type ] : B3D.UnsignedByteType;

							var samplers = json.samplers || {};
							var sampler = samplers[ texture.sampler ] || {};

							_texture.magFilter = WEBGL_FILTERS[ sampler.magFilter ] || B3D.LinearFilter;
							_texture.minFilter = WEBGL_FILTERS[ sampler.minFilter ] || B3D.NearestMipMapLinearFilter;
							_texture.wrapS = WEBGL_WRAPPINGS[ sampler.wrapS ] || B3D.RepeatWrapping;
							_texture.wrapT = WEBGL_WRAPPINGS[ sampler.wrapT ] || B3D.RepeatWrapping;
		...
		return this._withDependencies( [
			...
			return _each( json.materials, function ( material ) {
				...
				if ( materialExtensions[ EXTENSIONS.KHR_MATERIALS_COMMON ] ) {
					...
					materialType = B3D.MeshStandardMaterial;

					var metallicRoughness = material.pbrMetallicRoughness;

					materialParams.color = new B3D.Color( 1.0, 1.0, 1.0 );
					...
					materialType = B3D.MeshPhongMaterial;
				...
				if ( material.emissiveFactor !== undefined ) {
					...
					if ( materialType === B3D.MeshBasicMaterial ) {

						materialParams.color = new B3D.Color().fromArray( material.emissiveFactor );

					} else {

						materialParams.emissive = new B3D.Color().fromArray( material.emissiveFactor );
				...
				if ( materialType === B3D.ShaderMaterial ) {
			...
			return _each( json.meshes, function ( mesh ) {
				...
				for ( var name in primitives ) {
					...
					if ( primitive.mode === WEBGL_CONSTANTS.TRIANGLES || primitive.mode === undefined ) {
						...
						geometry = new B3D.BufferGeometry();
						...
						if ( material.aoMap !== undefined
							...
							geometry.addAttribute( 'uv2', new B3D.BufferAttribute( geometry.attributes.uv.array, 2 ) );

						}

						meshNode = new B3D.Mesh( geometry, material );
						...
						if ( primitive.indices !== undefined ) {
							...
							meshNode = new B3D.LineSegments( geometry, material );

						} else {

							meshNode = new B3D.Line( geometry, material );
					...
					if ( geometry.attributes.color !== undefined ) {
						...
						material.vertexColors = B3D.VertexColors;
	...
	GLTFParser.prototype.loadCameras = function () {
		...
		return _each( json.cameras, function ( camera ) {
			...
			if ( camera.type == "perspective" && camera.perspective ) {
				...
				var _camera = new B3D.PerspectiveCamera( B3D.Math.radToDeg( xfov ), aspectRatio, camera.perspective.znear || 1, camera.perspective.zfar || 2e6 );
				...
				var _camera = new B3D.OrthographicCamera( window.innerWidth / - 2, window.innerWidth / 2, window.innerHeight / 2, window.innerHeight / - 2, camera.orthographic.znear, camera.orthographic.zfar );
				if ( camera.name !== undefined ) _camera.name = camera.name;
	...
	GLTFParser.prototype.loadSkins = function () {
		...
		return this._withDependencies( [
			...
			return _each( json.skins, function ( skin ) {
				...
				var bindShapeMatrix = new B3D.Matrix4();
	...
	GLTFParser.prototype.loadAnimations = function () {
		...
		return this._withDependencies( [
			...
			return _each( json.animations, function ( animation, animationId ) {
						...
						if ( node ) {
							...
							switch ( PATH_PROPERTIES[ target.path ] ) {
								...
								case PATH_PROPERTIES.weights:
									...
									TypedKeyframeTrack = B3D.NumberKeyframeTrack;
									break;

								case PATH_PROPERTIES.rotation:

									TypedKeyframeTrack = B3D.QuaternionKeyframeTrack;
									break;
								...
								default:
									...
									TypedKeyframeTrack = B3D.VectorKeyframeTrack;
									break;

							}

							var targetName = node.name ? node.name : node.uuid;
							var interpolation = sampler.interpolation !== undefined ? INTERPOLATION[ sampler.interpolation ] : B3D.InterpolateLinear;

							var targetNames = [];

							if ( PATH_PROPERTIES[ target.path ] === PATH_PROPERTIES.weights ) {

								// node should be B3D.Group here but
							...
							for ( var i = 0, il = targetNames.length; i < il; i ++ ) {
								...
								tracks.push( new TypedKeyframeTrack(
									...
									B3D.AnimationUtils.arraySlice( inputAccessor.array, 0 ),
									B3D.AnimationUtils.arraySlice( outputAccessor.array, 0 ),
									interpolation
								) );
				...
				return new B3D.AnimationClip( name, undefined, tracks );
	...
	GLTFParser.prototype.loadNodes = function () {
		...
		return _each( json.nodes, function ( node ) {
			...
			if ( _node.name !== undefined ) {
				...
				_node.name = B3D.PropertyBinding.sanitizeNodeName( _node.name );
						...
						for ( var meshId in meshes ) {
							...
							for ( var childrenId in group.children ) {
								...
								switch ( child.type ) {
									...
									case 'LineSegments':
										child = new B3D.LineSegments( originalGeometry, material );
										break;

									case 'LineLoop':
										child = new B3D.LineLoop( originalGeometry, material );
										break;

									case 'Line':
										child = new B3D.Line( originalGeometry, material );
										break;

									default:
										child = new B3D.Mesh( originalGeometry, material );
								...
								if ( skinEntry ) {
									...
									child = new B3D.SkinnedMesh( geometry, material, false );
									child.castShadow = true;
									child.userData = originalUserData;
									child.name = originalName;
									...
									for ( var i = 0, l = skinEntry.joints.length; i < l; i ++ ) {
										...
										if ( jointNode ) {
											...
											var mat = new B3D.Matrix4().fromArray( m, i * 16 );
									...
									child.bind( new B3D.Skeleton( bones, boneInverses, false ), skinEntry.bindShapeMatrix );
