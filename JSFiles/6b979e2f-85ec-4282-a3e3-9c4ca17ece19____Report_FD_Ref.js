...
export function Report_FD_Ref({navigation}) {
  
    const {allRef, getResult, getResultGoods, Res_Ref, res_out,Res_In,FD_Ref, FD_out, FD_in, ewallet, Debit,Credit,Cash,Cancel_Res,All_Ref  } = useTasks();
    const [customers, setCustomer] = useState(allRef)
  ...
  const reports_info = allRef.map( ({check_in, check_out, room_no,room_type, customer,company,contact,control_num,discount,discount_code,discount_less, email, extension_total_amount, extension_person,extension_price,extension_rate,  nationality,extra_person, checkin_stat , hour_duration, no_person, no_person_discount, refund, note, penalty, penalty_description, res_code, total_addtional, temp_id}) => ({check_in, check_out, room_no,room_type, customer,company,contact,control_num,discount,discount_code,discount_less, email, extension_total_amount, extension_person,extension_price,extension_rate,  nationality,extra_person, checkin_stat , hour_duration, no_person, no_person_discount, refund, note, penalty, penalty_description, res_code, total_addtional,temp_id }) )
  ...
  const room_types = [...new Map(allRef.map(item => [item[key], item])).values()]
  const key_nat ="nationality"
const natl = [...new Map(allRef.map(item => [item[key_nat], item])).values()]
console.log('allRef: ',allRef.length)
  const requestRunTimePermission=()=>{
    ...
    const name = 'Refund ('+ date_now +' )'+'.xlsx'
 ...
 const searchData = (text) => {
  ...
  const newData = allRef.filter(item => {
  ...
  return (
   ...
   <View style={styles.mainContainer}>
          ...
          <Item>
            ...
            <Input placeholder="FD Refund Search Customer Name" style={{borderColor: 'red', fontSize: 13}}    onChangeText={(text) => searchData(text)}/>
...
<CardItem  style={{marginBottom: 5, paddingTop: -15, paddingBottom: -10,}}>
                                ...
                                <Body />
                                  ...
                                  <Right>
                                  <Text note style={{width: '100%', alignContent:'flex-end',fontSize: 14,}}>{currencyFormat(item.payment)} </Text>
                                  </Right>
            ...
            body={
              ...
              <Body>
                ...
                {customers && customers.length ? customers.reduce((sum, i) => (
                                      ...
                                      sum += parseFloat(i.refund)
                                    ), 0).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,') : allRef.reduce((sum, i) => (
...
{room_types.map((item) => 
              ...
              sum += parseFloat(i.room_type == item.room_type ? i.refund: 0)
            ), 0).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,') : allRef.reduce((sum, i) => (
...
<CardItem bordered>
              ...
              sum += parseFloat(i.payment_method=='Cash' || i.payment_method == ""? i.refund: 0)
            ), 0).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,') : allRef.reduce((sum, i) => (

              sum += parseFloat(i.payment_method=='Cash'? i.refund: 0)
            ), 0).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')}</Text>  
                                                     </Body>
                                                      </CardItem>
              ...
              sum += parseFloat(i.payment_method=='Debit Card'? i.refund: 0)
            ), 0).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,') : allRef.reduce((sum, i) => (
              ...
              sum += parseFloat(i.payment_method=='Credit Card'? i.refund: 0)
            ), 0).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,') : allRef.reduce((sum, i) => (
              ...
              sum += parseFloat(i.payment_method=='E-Wallet'? i.refund: 0)
            ), 0).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,') : allRef.reduce((sum, i) => (
              ...
              sum += parseFloat(i.checkin_stat== 'Over The Counter' || i.checkin_stat==""|| i.checkin_stat==null?i.refund:0)
            ), 0).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,') : allRef.reduce((sum, i) => (

              sum += parseFloat(i.checkin_stat== 'Over The Counter'?i.refund:0)
            ), 0).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')}</Text>  
                                                     </Body>
                                                      </CardItem>
...
<CardItem bordered style={{marginTop: -10}}>
              ...
              sum += parseFloat(i.checkin_stat== 'Reservation'?i.refund:0)
            ), 0).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,') : allRef.reduce((sum, i) => (
...
<CardItem bordered>
              ...
              sum += 1
                                                      ...
                                                      <CardItem style={{marginTop: 10}} >
                                                        <Left  style={{marginTop: -10, marginBottom: -10}}>
                                                          ...
                                                          <Body>
                                                            <Text style={{fontSize: 14, fontWeight: 'bold'}}>Front Desk Officer</Text>
                                                            </Body>
                                                        </Left>
                                                      ...
                                                      <CardItem key={item._id} style={{height: 35}}>
                                                        <Left  style={{marginTop: -10, marginBottom: -10}}>
                                                            ...
                                                            <Text style={{fontSize: 13, width: SCREEN_WIDTH/1.8}}>{item.CheckedInBy}</Text>
                                                           
                                                        </Left>
                                                        <Right>
                                                    <Text style={{fontSize: 13}}>{  customers && customers.length ? customers.reduce((sum, i) => (
              
              sum += parseFloat(i.CheckedInBy == item.CheckedInBy ? 1: 0)
              ...
              sum += parseFloat(i.CheckedInBy == item.CheckedInBy ? i.payment: 0)
