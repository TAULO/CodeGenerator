...
function validateNumber()
{
   ...
   var d = $('#codeDest').val()+$('#dest').val();
    ...
    if($('#source').val().length == 0 || $('#dest').val().length == 0)
    {
        return;
    }  
    var Regx = /^[0-9]{10,15}$/;

    if ( !Regx.test(s) || !Regx.test(d) ) 
    {
    ...
    if(s == d)
    {
        show_message("Source and Destination numbers could not be same", "error");
        return; 
    }    
...
var timeoutId = 0;
$(".sourceDestNo").bind('keypress keyup' ,function (e) 
{
    ...
    timeoutId = setTimeout(validateNumber, 1000);
