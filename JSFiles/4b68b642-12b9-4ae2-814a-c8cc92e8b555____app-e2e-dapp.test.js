...
async function signTransaction(txnParams, browserPage) {
    ...
    signedTx = await browserPage.evaluate(async (txnParams) => {
        ...
        var signPromise = MESE.sign(txnParams);
    ...
    test('Get TestNet accounts', async () => {
        const getAccounts = await appPage.evaluate( () => {
            ...
            return Promise.resolve(
                MESE.accounts({ledger: 'TestNet'})
                    .then((d) => {
                        console.log(`MESE Testnet Account reached: ${d}`)
                        return d;
                    })
                    .catch((e) => {
        ...
        console.log(`MESE Testnet Account reached: ${getAccounts[0].address}`)
