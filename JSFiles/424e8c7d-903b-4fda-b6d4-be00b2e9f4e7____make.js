...
const crlfchecker = require('./external/crlfchecker/crlfchecker.js');
...
const ROOT_DIR = `${__dirname}/`; // absolute path to project's root
const BUILD_DIR = 'build/';
const SRC_DIR = 'src/';
const BUILD_TARGET = `${BUILD_DIR}pdf.js`;
const BUILD_WORKER_TARGET = `${BUILD_DIR}pdf.worker.js`;
const BUILD_TARGETS = [BUILD_TARGET, BUILD_WORKER_TARGET];
const FIREFOX_BUILD_DIR = `${BUILD_DIR}/firefox/`;
const CHROME_BUILD_DIR = `${BUILD_DIR}/chromium/`;
const B2G_BUILD_DIR = `${BUILD_DIR}/b2g/`;
const JSDOC_DIR = `${BUILD_DIR}jsdoc`;
const EXTENSION_SRC_DIR = 'extensions/';
const FIREFOX_CONTENT_DIR = `${EXTENSION_SRC_DIR}/firefox/content/`;
const LOCALE_SRC_DIR = 'l10n/';
const GH_PAGES_DIR = `${BUILD_DIR}gh-pages/`;
const GENERIC_DIR = `${BUILD_DIR}generic/`;
const MINIFIED_DIR = `${BUILD_DIR}minified/`;
const SINGLE_FILE_DIR = `${BUILD_DIR}singlefile/`;
const COMPONENTS_DIR = `${BUILD_DIR}components/`;
const REPO = 'git@github.com:mozilla/pdf.js.git';
const MOZCENTRAL_PREF_PREFIX = 'pdfjs';
const FIREFOX_PREF_PREFIX = 'extensions.uriloader@pdf.js';
const MOZCENTRAL_STREAM_CONVERTER_ID = 'd0c5195d-e798-49d4-b1d3-9324328b2291';
const FIREFOX_STREAM_CONVERTER_ID = '6457a96b-2d68-439a-bcfa-44465fbcdbb1';
const MOZCENTRAL_STREAM_CONVERTER2_ID = 'd0c5195d-e798-49d4-b1d3-9324328b2292';
const FIREFOX_STREAM_CONVERTER2_ID = '6457a96b-2d68-439a-bcfa-44465fbcdbb2';
...
const COMMON_WEB_FILES =
      ['web/images',
        'web/debugger.js'];
const COMMON_WEB_FILES_PREPROCESS =
      ['web/viewer.js',
        'web/viewer.html'];
const COMMON_FIREFOX_FILES_PREPROCESS =
...
target.components = function () {
  ...
  const defines = builder.merge(DEFINES, {COMPONENTS: true});
...
target.dist = function () {
  ...
  cd(DIST_DIR);
  const reason = process.env.PDFJS_UPDATE_REASON;
  const message = `PDF.js version ${VERSION}${reason ? ` - ${reason}` : ''}`;
  exec('git add *');
...
target.publish = function () {
  ...
  cd(GENERIC_DIR);
  const distFilename = `pdfjs-${VERSION}-dist.zip`;
...
target.locale = function () {
  const METADATA_OUTPUT = 'extensions/firefox/metadata.inc';
  const CHROME_MANIFEST_OUTPUT = 'extensions/firefox/chrome.manifest.inc';
  const EXTENSION_LOCALE_OUTPUT = 'extensions/firefox/locale';
  ...
  const subfolders = ls(LOCALE_SRC_DIR);
  subfolders.sort();
  let metadataContent = '';
  let chromeManifestContent = '';
  let viewerOutput = '';
  for (let i = 0; i < subfolders.length; i++) {
    const locale = subfolders[i];
    const path = LOCALE_SRC_DIR + locale;
    if (!test('-d', path)) {
      continue;
    }
    ...
    if (test('-f', `${path}/metadata.inc`)) {
      const metadata = cat(`${path}/metadata.inc`);
...
target.bundle = function (args) {
  args = args || {};
  const defines = args.defines || DEFINES;
  ...
  function bundle(filename, outfilename, SRC_FILES, EXT_SRC_FILES) {
    for (let i = 0, length = excludes.length; i < length; ++i) {
      const exclude = excludes[i];
      const index = SRC_FILES.indexOf(exclude);
      if (index >= 0) {
        SRC_FILES.splice(index, 1);
      }
    }

    let bundleContent = cat(SRC_FILES);
    const bundleVersion = VERSION;
    const bundleBuild = exec('git log --format="%h" -n 1',
  ...
  let MAIN_SRC_FILES = SHARED_SRC_FILES.concat([
    'display/api.js',
    'display/metadata.js',
    'display/canvas.js',
    'display/webgl.js',
  ...
  const srcFiles = builder.getWorkerSrcFiles('src/worker_loader.js');
  let WORKER_SRC_FILES = srcFiles.srcFiles;
  ...
  const EXT_SRC_FILES = srcFiles.externalSrcFiles;

  cd(SRC_DIR);

  bundle('pdf.js', ROOT_DIR + BUILD_TARGET, MAIN_SRC_FILES, []);
  const srcCopy = `${ROOT_DIR + BUILD_DIR}pdf.worker.js.temp`;
  cp('pdf.js', srcCopy);
...
target.singlefile = function () {
  ...
  const SINGLE_FILE_TARGET = `${BUILD_DIR}pdf.combined.js`;

  const defines = builder.merge(DEFINES, {SINGLE_FILE: true});
  target.bundle({defines});
...
target.minified = function () {
  const compilerPath = process.env.CLOSURE_COMPILER;
  if (!compilerPath) {
  ...
  const defines = builder.merge(DEFINES, {GENERIC: true, MINIFIED: true});
  ...
  const cmdPrefix = `java -jar \"${compilerPath}\" ` +
  ...
  exec(`${cmdPrefix + viewerFiles.map((s) => `--js \"${s}\"`).join(' ')
...
target.buildnumber = function () {
  ...
  const lines = exec(`git log --format=oneline ${
...
target.firefox = function () {
  ...
  const defines = builder.merge(DEFINES, {FIREFOX: true});

  const FIREFOX_BUILD_CONTENT_DIR = `${FIREFOX_BUILD_DIR}/content/`;
  const FIREFOX_EXTENSION_DIR = 'extensions/firefox/';
  const FIREFOX_EXTENSION_FILES_TO_COPY =
        ['*.js',
          '*.rdf',
          '*.svg',
          '*.png',
  ...
  const FIREFOX_EXTENSION_FILES =
        ['bootstrap.js',
          ...
          'locale',
          'LICENSE'];
  const FIREFOX_EXTENSION_NAME = 'pdf.js.xpi';
  const FIREFOX_AMO_EXTENSION_NAME = 'pdf.js.amo.xpi';

  target.locale();
  target.bundle({excludes: ['core/network.js'], defines});
  cd(ROOT_DIR);
  ...
  find(FIREFOX_BUILD_DIR).forEach((file) => {
    if (file.match(/^\./)) {
      rm('-f', file);
    }
  });
  ...
  const localizedMetadata = cat(`${EXTENSION_SRC_DIR}/firefox/metadata.inc`);
  sed('-i', /.*PDFJS_LOCALIZED_METADATA.*\n/, localizedMetadata,
      `${FIREFOX_BUILD_DIR}/install.rdf`);
  const chromeManifest = cat(`${EXTENSION_SRC_DIR}/firefox/chrome.manifest.inc`);
...
target.mozcentral = function () {
  ...
  const FIREFOX_MC_EXCLUDED_FILES =
        ['icon.png',
          'icon64.png'];
  ...
  // Remove '.DS_Store' and other hidden files
  find(MOZCENTRAL_DIR).forEach((file) => {
    if (file.match(/^\./)) {
      rm('-f', file);
    }
  });
  ...
  FIREFOX_MC_EXCLUDED_FILES.forEach((file) => {
    if (test('-f', file)) {
      rm('-r', file);
    }
  });
...
target.b2g = function () {
  ...
  const B2G_BUILD_CONTENT_DIR = `${B2G_BUILD_DIR}/content/`;
  target.bundle();
...
target.chromium = function () {
  ...
  const defines = builder.merge(DEFINES, {CHROME: true});

  const CHROME_BUILD_DIR = `${BUILD_DIR}/chromium/`;
  const CHROME_BUILD_CONTENT_DIR = `${CHROME_BUILD_DIR}/content/`;
  ...
  const file_list = ls('-RA', CHROME_BUILD_CONTENT_DIR);
  const public_chrome_files = file_list.reduce((war, file) => {
  ...
  const pem = env.PDFJS_CHROME_KEY;
  if (!pem) {
    return;
  }
  ...
  const browserManifest = env.PDF_BROWSERS ||
...
target.browsertest = function (options) {
  ...
  const PDF_TEST = env.PDF_TEST || 'test_manifest.json';
  const PDF_BROWSERS = env.PDF_BROWSERS ||
  ...
  const reftest = (options && options.noreftest) ? '' : '--reftest';
...
target.baseline = function () {
  ...
  const baselineCommit = env.BASELINE;
  if (!baselineCommit) {
...
target.mozcentraldiff = function () {
  ...
  const MOZCENTRAL_DIFF = `${BUILD_DIR}mozcentral.diff`;
...
target.mozcentralcheck = function () {
  ...
  const MOZCENTRAL_DIFF = `${BUILD_DIR}mozcentral_changes.diff`;
  if (test('-f', MOZCENTRAL_DIFF)) {
    rm(MOZCENTRAL_DIFF);
  }
  ...
  cd('..');
  exec('git add -A');
  const diff = exec('git diff --binary --cached --unified=8',
      {silent: true}).output;
...
target.lint = function () {
  ...
  const jshintPath = path.normalize('./node_modules/.bin/jshint');
  // Lint the Firefox specific *.jsm files.
  const options = '--extra-ext .jsm';

  const exitCode = exec(`"${jshintPath}" ${options} .`).code;
  if (exitCode === 0) {
