var TrustExtensionNameSpace = {
    ...
    TEShowInit: function () {
        ...
        if (self.TEShowInfo.length > 0) {
            ...
            //console.log("This is JSRList.HaveDataList");
            //console.log(self.TrustExtensionData.JSRList.HaveDataList);
        }
    },
    ...
    //默认升序排序
    initSortDate:function(){
        var self = TrustExtensionNameSpace;
        self.sortDate(self, false);
    },
    sortDate:function(self,order){        
        var temp = new Array();
        var temp = temp.concat(self.TrustExtensionData.JSRList.HaveDataList());
        temp = self.SortDateByOrder(temp, order);
        //先全部删除再重新添加的方式重新渲染
        self.TrustExtensionData.JSRList.HaveDataList.removeAll();
...
var TrustPeriod = (function () {
    ...
    function ExecuteRemoteDataPost(executeParam) {
        ...
        $.ajax({
            ...
            type: "POST",
            async: false,
            url: "https://poolcutwcf/TaskProcessServices/DataProcessService.svc/jsAccessEP/CommonExecuteGet",
            dataType: "json",
            contentType: "application/json;charset=utf-8",
            data: executeParams,
            success: function (response) {
