...
const extensionPACKAGE_JSON = require(path.join(extension_root_dir, 'package.json'));

// const periodicClass = require('periodicjs/lib/periodicClass');
const testPathDir = path.resolve(__dirname, '../mock/spec/periodic');
const initTestPathDir = path.join(testPathDir, 'EXT_TEST');
// const initTestConfigJsonFile = path.join(initTestPathDir, 'content/config/config.json');
// const extTestConfigJson = require('../../mock/config/config_test_config');
let extPeriodic = require('periodicjs');

describe('Valid Periodic extension', function() {
  this.timeout(10000);
  before('initialize extension test periodic dir', (done) => {
    Promise.all([
        fs.ensureDir(initTestPathDir),
        fs.ensureDir(path.join(initTestPathDir, 'node_modules'))
      ])
      .then(() => {
        // extPeriodic = new periodicClass({});
        // console.log('extPeriodic.init', extPeriodic.init)
        extPeriodic.init({
