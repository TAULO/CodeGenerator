...
btnStartRecording.onclick = function(event) {
    ...
    if(button.innerHTML === 'Stop Recording') {
        ...
        button.innerHTML = 'Test My System';
    ...
    var commonConfig = {
        onMediaCaptured: function(stream) {
            $('.media-box h2').css('display','block');
            start_test_interview();
...
function saveToDiskOrOpenNewTab(recordRTC) {
    ...
    $('#open-new-tab').css('display','initial');
...
function uploadToPHPServer(fileName, recordRTC, callback) {
    ...
    makeXMLHttpRequest('upload', formData, function(progress) {
        ...
        var initialURL = location.origin+'/login/application/controllers/interview/uploads/';
