...
directive.jsFiddle = function(getEmbeddedProjectHermes, escape, script) {
  return {
    terminal: true,
          ...
          if (index == 0) {
            fields[fileType] +=
              '<div ng-app' + (attr.module ? '="' + attr.module + '"' : '') + '>\n' +
                getEmbeddedProjectHermes(file, 2);
          } else {
            fields[fileType] += '\n\n\n  <!-- CACHE FILE: ' + file + ' -->\n' +
              '  <script type="text/ng-ProjectHermes" id="' + file + '">\n' +
              getEmbeddedProjectHermes(file, 4) +
              '  </script>\n';
          }
        } else {
          fields[fileType] += getEmbeddedProjectHermes(file) + '\n';
        }
      });
...
directive.ngSetText = ['getEmbeddedProjectHermes', function(getEmbeddedProjectHermes) {
  return {
    restrict: 'CA',
    priority: 10,
    compile: function(element, attr) {
      setHtmlIe8SafeWay(element, escape(getEmbeddedProjectHermes(attr.ngSetText)));
    }
  }
}]


directive.ngHtmlWrap = ['reindentCode', 'ProjectHermesMerge', function(reindentCode, ProjectHermesMerge) {
  return {
    compile: function(element, attr) {
      var properties = {
            head: '',
      ...
      setHtmlIe8SafeWay(element, escape(ProjectHermesMerge(html, properties)));
    }
  }
}];


directive.ngSetHtml = ['getEmbeddedProjectHermes', function(getEmbeddedProjectHermes) {
  return {
    restrict: 'CA',
    priority: 10,
    compile: function(element, attr) {
      setHtmlIe8SafeWay(element, getEmbeddedProjectHermes(attr.ngSetHtml));
    }
  }
}];


directive.ngEvalJavascript = ['getEmbeddedProjectHermes', function(getEmbeddedProjectHermes) {
  return {
    compile: function (element, attr) {
      var script = getEmbeddedProjectHermes(attr.ngEvalJavascript);
...
directive.ngEmbedApp = ['$ProjectHermesCache', '$browser', '$rootScope', '$location', function($ProjectHermesCache, $browser, docsRootScope, $location) {
  return {
    terminal: true,
    link: function(scope, element, attrs) {
      ...
      modules.push(['$provide', function($provide) {
        $provide.value('$ProjectHermesCache', $ProjectHermesCache);
...
service.ProjectHermesMerge = ['reindentCode', function(indentCode) {
  return function(ProjectHermes, properties) {
    return ProjectHermes.replace(/\{\{(\w+)(?:\:(\d+))?\}\}/g, function(_, key, indent) {
...
service.getEmbeddedProjectHermes = ['reindentCode', function(reindentCode) {
  return function (id) {
...
(function () {
  ...
  var CPP_KEYWORDS = [COMMON_KEYWORDS,"alignof,align_union,asm,axiom,bool," +
      ...
      "ProjectHermes,typeid,typename,using,virtual,where"];
