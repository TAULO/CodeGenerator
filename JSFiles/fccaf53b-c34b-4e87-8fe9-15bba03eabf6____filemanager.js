 ...
 *	@author		Stefano Gualdi <stefano (dot) gualdi (at) gmail (dot) com> (Grails customization)
 *	@copyright	Authors
 *
...
var loadConfigFile = function (type) {
	...
	if(type == 'user') {
		...
		var url = ofmBase + '/scripts/filemanager.config.js.default'
	}
...
var moveItem = function(data) {
	...
	var doMove = function(v, m){
		...
		if(rname != ''){
			var givenName = rname;
			var oldPath = data['Path'];
			var connectString = fileConnector + '?mode=move&old=' + encodeURIComponent(data['Path']) + '&space=' + space + '&type=' + type + '&new=' + encodeURIComponent(givenName) + '&root=' + encodeURIComponent(fileRoot);
...
var deleteItem = function(data) {
	...
	var doDelete = function(v, m){
		if(v != 1) return false;
    var d = new Date(); // to prevent IE cache issues
		var connectString = fileConnector + '?mode=delete&path=' + encodeURIComponent(data['Path']) + '&space=' + space + '&type=' + type  + '&time=' + d.getMilliseconds();
...
var editItem = function(data) {
	...
	isEdited = false;
		...
		$('#edit-file').click(function() {
					...
					var d = new Date(); // to prevent IE cache issues
					var connectString = fileConnector + '?mode=editfile&path=' + encodeURIComponent(data['Path']) + '&space=' + space + '&type=' + type + '&time=' + d.getMilliseconds();
	...
	$.getJSON(fileConnector + '?mode=getinfo&path=' + path + '&space=' + space + '&type=' + type + '&time=' + d.getMilliseconds(), function(data){
		...
		switch(action){
			...
			case 'download': // todo implement javascript method to test if exstension is correct
				window.location = fileConnector + '?mode=download&path=' + data['Path'] + '&space=' + space + '&type=' + type + '&time=' + d.getMilliseconds();
				break;
...
var getFolderInfo = function(path) {
	...
	var d = new Date(); // to prevent IE cache issues
	var url = fileConnector + '?path=' + encodeURIComponent(path) + '&mode=getfolder&showThumbs=' + config.options.showThumbs + '&space=' + space + '&type=' + type + '&time=' + d.getMilliseconds();
...
$(function(){
	...
	if(config.edit.enabled) {
		loadCSS(ofmBase + '/scripts/CodeMirror/lib/codemirror.css');
		loadCSS(ofmBase + '/scripts/CodeMirror/theme/' + config.edit.theme + '.css');
		loadJS(ofmBase + '/scripts/CodeMirror/lib/codemirror.js');
		loadJS(ofmBase + '/scripts/CodeMirror/addon/selection/active-line.js');
		loadJS(ofmBase + '/scripts/CodeMirror/dynamic-mode.js');
	}
	...
	/** load searchbox */
	if(config.options.searchBox === true)  {
		$.getScript(ofmBase + "/scripts/filemanager.liveSearch.min.js");
	} else {
		$('#search').remove();
	}
